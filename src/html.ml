...
let tag name ?cls ?id ?(attrs = []) t =
  let attrs = match id with None -> attrs | Some i -> ("id", i) :: attrs in
  let attrs =
...
let add_oattr name attr attrs =
  match attr with None -> attrs | Some i -> (name, i) :: attrs

let add_uattr name attr attrs =
  match attr with None -> attrs | Some i -> (name, Uri.to_string i) :: attrs

let add_iattr name attr attrs =
  match attr with None -> attrs | Some i -> (name, string_of_int i) :: attrs

let add_battr name attr attrs = if attr then (name, name) :: attrs else attrs

let input ?cls ?id ?(attrs = []) ?ty v =
  let attrs = ("value", v) :: add_oattr "type" ty attrs in
...
let source ?media ?ty uri =
  let attrs = add_oattr "media" media [] |> add_oattr "type" ty in
  tag "source" empty ~attrs:(("src", Uri.to_string uri) :: attrs)

let wbr = tag "wbr" empty
let param ~name v = tag "param" empty ~attrs:[ ("name", name); ("value", v) ]
...
let track ?(default = false) ?label kind uri =
  let attrs = add_battr "default" default [] |> add_oattr "label" label in
  let attrs =
    match kind with
    ...
    | `Subtitles lang -> ("kind", "subtitles") :: ("srclang", lang) :: attrs
  in
...
let keygen ?(autofocus = false) ?(disabled = false) ?form ?(challenge = true)
    ?(keytype = `RSA) name =
  let attrs =
    add_battr "autofocus" autofocus []
    |> add_battr "disabled" disabled
    |> add_oattr "form" form
    |> add_battr "challenge" challenge
  in
  let attrs =
    match keytype with
...
let link ?cls ?id ?(attrs = []) ?title ?media ?ty ?rel ?integrity ?crossorigin
    href =
  let attrs =
    add_oattr "media" media attrs
...
let meta ?cls ?id ?name ?content ?charset attrs =
  tag "meta" empty ?cls ?id
    ~attrs:
      (add_oattr "name" name
         (add_oattr "content" content (add_oattr "charset" charset attrs)))

let blockquote ?cls ?id ?(attrs = []) ?cite x =
  tag "blockquote" ?cls ?id ~attrs:(add_uattr "cite" cite attrs) x

let figure ?cls ?id ?(attrs = []) ?figcaption x =
  let x =
    match figcaption with None -> x | Some i -> tag "figcaption" Xml.(i ++ x)
  in
...
let dfn ?cls ?id ?(attrs = []) ?title x =
  tag "dfn" ?cls ?id ~attrs:(add_oattr "title" title attrs) x

let abbr ?cls ?id ?(attrs = []) ?title x =
  tag "abbr" ?cls ?id ~attrs:(add_oattr "title" title attrs) x

let data ?cls ?id ?(attrs = []) ~value x =
  tag "data" ?cls ?id ~attrs:(("value", value) :: attrs) x

let time ?cls ?id ?(attrs = []) ?datetime x =
  tag "time" ?cls ?id ~attrs:(add_oattr "datetime" datetime attrs) x
...
let ins ?cls ?id ?(attrs = []) ?cite ?datetime x =
  tag "ins" ?cls ?id
    ~attrs:(add_oattr "datetime" datetime (add_uattr "cite" cite attrs))
    x
...
let nil = empty
let concat = list
let li ?cls ?id ?attrs x = tag ?cls ?id ?attrs "li" x
let dt ?cls ?id ?attrs x = tag ?cls ?id ?attrs "dt" x
let dd ?cls ?id ?attrs x = tag ?cls ?id ?attrs "dd" x
...
let dl ?(add_dtdd = true) ?cls ?id ?attrs ?dtcls ?ddcls lss =
  let lss =
    if add_dtdd then
      List.map (fun (t, d) -> list [ dt ?cls:dtcls t; dd ?cls:ddcls d ]) lss
    else List.map (fun (t, d) -> list [ t; d ]) lss
  in
...
let output_doc ?(nl = false) ?(indent = None) ?(ns_prefix = fun _ -> None) dest
    t =
...
let of_string ?enc str = Xml.of_string ~entity:Xhtml.entity ?enc str
...
let a ?cls ?(attrs = []) ?hreflang ?rel ?target ?ty ?title ?href html =
  ...
  in
  let attrs =
    match ty with Some t -> (("", "type"), t) :: attrs | None -> attrs
  in
  let attrs =
    match title with Some t -> (("", "title"), t) :: attrs | None -> attrs
  in
  let attrs =
    match cls with Some c -> (("", "class"), c) :: attrs | None -> attrs
  in
  [ `El ((("", "a"), attrs), html) ]
...
let img ?alt ?width ?height ?ismap ?title ?cls ?crossorigin ?(attrs = []) src =
  ...
  let attrs =
    match alt with Some t -> (("", "alt"), t) :: attrs | None -> attrs
  in
  let attrs =
    match width with
...
let style ?media ?(scoped = false) css =
  let attrs = add_oattr "media" media [] |> add_battr "scoped" scoped in
...
let html_of_table ?(headings = false) t =
  ...
  let tl =
    ...
    else List.map Array.to_list (Array.to_list t)
  in
...
module Create = struct
    ...
    let color_of_string ?(fmt = `Hex) s =
      ...
      let rval =
        ...
        | `Hex ->
            ...
            let r, g, b =
              ((x land 0xff0000) lsr 16, (x land 0xff00) lsr 8, x land 0xff)
            in
  ...
  let table ?(flags = [ Headings_fst_row ]) =
    ...
    let aux ~row tbl =
      ...
      let rows =
        if !side then
          List.mapi (fun i _ -> List.map (fun el -> List.nth el i) rows)
          @@ List.hd rows
        else rows
      in
      ...
      let rows =
        ...
        | false, false -> tbody (tr (cellify rows))
        | true, false ->
