...
let gen_xml (_loc, n, (t_exp:Dyntype.Type.t)) =
  let open Dyntype.Type in
  ...
  let rec aux id = function
    ...
    | Var n    ->
      ...
      <:expr< $Pa_dyntype.P4_type.gen_ident _loc xml_of n$ $id$ >>
  in
  ...
  Pa_type_conv.add_generator "xml"
    (fun _ tds ->
      try
        ...
        <:str_item<
          value rec $Ast.biAnd_of_list (List.map gen_xml (Pa_dyntype.P4_type.create tds))$;
        >>
      with Not_found ->
